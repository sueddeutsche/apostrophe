apos.define('apostrophe-schemas', {
  ...
  construct: function(self, options) {
    ...
    self.findFieldset = function($el, name) {
      // Use of :first sidesteps problems with other uses of data-name provided they are
      // consistently referring to the field name. Use of findSafe prevents us from
      // returning nested fields
      return self.findSafe($el, '[data-name="' + name + '"]:first');
    };
    ...
    self.addFieldType({
      ...
      populate: function(data, name, $field, $el, field, callback) {
        ...
        $field = self.findFieldset($el, name);

        var chooseInfo = { type: field.name };

        apos.emit('findChooser', chooseInfo);

        var $choose = $el.find('[data-choose]');
        if (!chooseInfo.chooser) {
          $choose.hide();
        }

        $field.off('click.choose', '[data-choose]');
        $field.on('click.choose', '[data-choose]', function() {
          if (chooseInfo.chooser) {
            var subset = [ field ];
            // Simplified universal interface for editing one-to-one or
            // one-to-many joins via a chooser. This is equivalent to
            // what we'd pass to jquery selective, but jquery selective is
            // not involved
            var input = {
              items: return _.map(data[field.idsField] || [], function(id) {
                var item = { value: id };
                _.assign(item, (data[field.relationshipsField] || {})[id]);
              }),
            ...
            return async.series({
              choose: function(callback) {
                return chooseInfo.chooser(input, function(err, _output) {
                  if (err) {
                    return callback(err);
                  }
                  output = _output;
              ...
              populate: function(callback) {
                data[field.idsField] = _.pluck(output.items, '_id');
                if (field.relationshipsField]) {
                  var relationships = {};
                  _.each(output.items, function(item) {
                    relationships[item._id] = _.omit(item, '_id');
                  });
                  data[field.relationshipsField] = relationships;
                }
                return self.populate($el, subset, data, callback);
              }
            }, function(err) {
              if (err) {
                // TODO: is there any useful error we could display here?
                apos.log(err);
              }
            });
          }
