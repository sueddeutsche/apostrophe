...
module.exports = function(self) {
  ...
  self.addMigration('normalizePersonGroupRelationship', function normalizePersonGroupRelationship(callback) {
    var groupsToPeople = {};
    var needed = false;
    return async.series({
      findRelationships: function(callback) {
        return self.forEachPage({ type: 'person', groupIds: { $exists: 1 } }, function(person, callback) {
          needed = true;
          var oldExtras = person.groupExtras;
          _.each(person.groupIds, function(groupId) {
            if (!groupsToPeople[groupId]) {
              groupsToPeople[groupId] = { personIds: [], personExtras: {} };
            }
            var rank = oldExtras[groupId] && oldExtras[groupId].rank;
            groupsToPeople[groupId].personIds.push([ rank, person._id ]);
            groupsToPeople[groupId].personExtras[person._id] = _.omit(oldExtras[groupId] || {}, 'rank');
          });
          var update = { $unset: { groupIds: 1, groupExtras: 1 }, $set: { oldGroupIds: person.groupIds, oldGroupExtras: person.groupExtras } };
          console.log('I would update person ' + person._id + ' to ' + JSON.stringify(update));
          // return setImmediate(callback);
          return self.pages.update({ _id: person._id }, { $unset: { groupIds: 1, groupExtras: 1 }, $set: { oldGroupIds: person.groupIds, oldGroupExtras: person.groupExtras } }, callback);
        }, callback);
      },
      setRelationships: function(callback) {
        if (!needed) {
          return callback(null);
        }
        console.log('Migrating group/person relationships from people to groups...');
        return async.eachSeries(_.keys(groupsToPeople), function(id, callback) {
          var gp = groupsToPeople[id];
          gp.personIds.sort(function(a, b) {
            if (a[0] < b[0]) {
              return -1;
            } else if (a[0] > b[0]) {
              return 1;
          ...
          gp.personIds = _.pluck(gp.personIds, 1);
          console.log('I would set ' + id + ' to ' + JSON.stringify(gp));
          // return setImmediate(callback);
          return self.pages.update({ _id: id }, { $set: gp }, callback);
        }, callback);
      }
    }, callback);
  });
